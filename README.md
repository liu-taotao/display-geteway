...
03.主要就是mqtt库移植到arm开发板如何移植呢？(还在找方法呢)
------------------------------------------------------
...
* ①主要是主机 `询问` 从机 `应答`方式 【已经能成功接收了】同时发现了一个[CRC校验网站](http://www.ip33.com/crc.html) 只需将modbus发送数据前6个输入 就会自动生成`CRC校验位` 注意这里使用时 这两位要交换。
